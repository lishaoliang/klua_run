...
-- @file   make/ch2_s2_3.lua
-- @brief  kpfs.image.create fixed vhdx + image.info size (2.2.3)
...
function M.run(...)
	...
	local work, err = common.setup_work("2.2.3")
	if work == nil then
		common.fail(TAG, err)
		return
	end

	local image_path = paths.join(work, "fixed.vhdx")
