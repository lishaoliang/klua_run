...
-- @file   mtd/ch2_s6_8.lua
-- @brief  mtd.mkfs romfs (2.6.8)
-- @history 修改历史
--  \n 2026 创建文件
--]]
...
function M.run(...)
	...
	local work, err = mtd_common.setup_work("2.6.8")
	if work == nil then
	...
	local rc, msg = mtd.mkfs(path, { fs = "romfs" })
	if rc ~= 0 then
		mtd_common.fail(TAG, "mkfs rc=" .. tostring(rc))
		return
	end

	local ok, err_v = mtd_common.verify_mounted_fs(path, "romfs")
	if not ok then
